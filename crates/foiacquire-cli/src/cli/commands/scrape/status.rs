...
use foiacquire::models::{DocumentStatus, ServiceStatus};
...
async fn display_status_simple(settings: &Settings) -> anyhow::Result<()> {
    ...
    let active_services: Vec<_> = data
        .services
        .iter()
        .filter(|s| s.status != foiacquire::models::ServiceState::Stopped)
        .collect();
    ...
    if !active_services.is_empty() {
        ...
        for svc in &active_services {
            let status_style = match svc.status {
                foiacquire::models::ServiceState::Running => style(svc.status.as_str()).green(),
                foiacquire::models::ServiceState::Starting => style(svc.status.as_str()).yellow(),
                foiacquire::models::ServiceState::Error => style(svc.status.as_str()).red(),
                foiacquire::models::ServiceState::Idle => style(svc.status.as_str()).dim(),
...
fn draw_status(frame: &mut Frame, data: &StatusData) {
    ...
    if !active_services.is_empty() {
        ...
        let rows = active_services.iter().map(|svc| {
            let status_style = match svc.status {
                foiacquire::models::ServiceState::Running => Style::default().fg(Color::Green),
                foiacquire::models::ServiceState::Starting => Style::default().fg(Color::Yellow),
                foiacquire::models::ServiceState::Error => Style::default().fg(Color::Red),
                foiacquire::models::ServiceState::Idle => Style::default().fg(Color::DarkGray),
                _ => Style::default(),
            };
            let age = chrono::Utc::now() - svc.last_heartbeat;
            let age_str = if age.num_seconds() < 60 {
