...
use foia_models::{ServiceState, ServiceStatus, ServiceType};
...
mod tests {
    use super::*;
    use crate::diesel_context::DieselDbContext;
    ...
    async fn test_stats_persistence() {
        ...
        let mut status = ServiceStatus::new_scraper("test");
        status.update_scraper_stats(foia_models::ScraperStats {
        ...
        let retrieved = repo.get(&status.id).await.unwrap().unwrap();
        let stats: foia_models::ScraperStats = serde_json::from_value(retrieved.stats).unwrap();
