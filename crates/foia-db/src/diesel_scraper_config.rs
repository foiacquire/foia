...
impl DieselScraperConfigRepository {
    ...
    pub async fn get<T: serde::de::DeserializeOwned>(&self, source_id: &str) -> Result<Option<T>, DieselError> {
        ...
        match record {
            Some(r) => {
                let config: T = serde_json::from_str(&r.config)
    ...
    pub async fn get_all<T: serde::de::DeserializeOwned>(&self) -> Result<Vec<(String, T)>, DieselError> {
    ...
    pub async fn upsert(&self, source_id: &str, config: &impl serde::Serialize) -> Result<(), DieselError> {
...
mod tests {
    ...
    async fn test_scraper_config_crud() {
        ...
        assert!(repo.get::<serde_json::Value>("test-source").await.unwrap().is_none());
        ...
        // Verify retrieval
        assert!(!repo.is_empty().await.unwrap());
        let retrieved: serde_json::Value = repo.get("test-source").await.unwrap().unwrap();
        assert_eq!(retrieved["name"].as_str(), Some("Test Source"));
        assert_eq!(retrieved["base_url"].as_str(), Some("https://example.com"));
        ...
        // Get all
        let all: Vec<(String, serde_json::Value)> = repo.get_all().await.unwrap();
        assert_eq!(all.len(), 1);
        ...
        let retrieved: serde_json::Value = repo.get("test-source").await.unwrap().unwrap();
        assert_eq!(retrieved["name"].as_str(), Some("Updated Source"));
