...
//! For SQLite, all operations are wrapped in spawn_blocking since diesel-async
...
use super::diesel_pool::{create_diesel_pool, create_diesel_pool_from_url, run_blocking, SqlitePool};
...
impl DieselDbContext {
    ...
    ///
    /// This opens a connection pool to the database and runs schema initialization.
    pub fn new(db_path: &Path, documents_dir: &Path) -> Result<Self, diesel::r2d2::PoolError> {
        let pool = create_diesel_pool(db_path)?;
    ...
    pub fn from_url(database_url: &str, documents_dir: &Path) -> Result<Self, diesel::r2d2::PoolError> {
        let pool = create_diesel_pool_from_url(database_url)?;
    ...
    pub async fn init_schema(&self) -> Result<(), diesel::result::Error> {
    ...
    pub async fn list_tables(&self) -> Result<Vec<String>, diesel::result::Error> {
        let pool = self.pool.clone();
...
mod tests {
    ...
    async fn test_diesel_context() {
        ...
        let ctx = DieselDbContext::new(&db_path, &docs_dir).unwrap();
