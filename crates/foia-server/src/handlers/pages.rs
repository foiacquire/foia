...
/// Per-backend extraction result for a page.
...
pub async fn api_document_pages(
    ...
    let mut extractions_map: std::collections::HashMap<i64, Vec<ExtractionResult>> =
        std::collections::HashMap::new();
    for (page_id, ocr_results) in all_ocr_results {
        let extractions: Vec<ExtractionResult> = ocr_results
            .into_iter()
            ...
            .collect();
        extractions_map.insert(page_id, extractions);
    }
    ...
    let page_data_list: Vec<PageData> = if is_pdf {
        ...
        for page in selected_pages {
            ...
            let page_id = page.id;
            let search_text = page.search_text;
            let ocr_status = page.ocr_status.as_str().to_string();
            let extractions = extractions_map.remove(&page_id).unwrap_or_default();
        ...
        selected_pages
            .into_iter()
            .map(|page| {
                let extractions = extractions_map.remove(&page.id).unwrap_or_default();
                PageData {
